...
class WebRtcVoiceMediaChannel::WebRtcAudioReceiveStream {
 public:
  ...
  uint32_t GetPlayoutTimestamp() {
    webrtc::internal::AudioReceiveStream* strm =
        (webrtc::internal::AudioReceiveStream*)stream_;
    return strm->GetPlayoutTimestamp();
  }

  bool GetCurrentDelay(int& delay) {
    webrtc::internal::AudioReceiveStream* strm =
        (webrtc::internal::AudioReceiveStream*)stream_;
    absl::optional<webrtc::Syncable::Info> info = strm->GetInfo();
    if (!info) {
      RTC_LOG(LS_ERROR) << "WebRtcAudioReceiveStream::GetCurrentDelay, no info";
      delay = 0;
      return false;
	}
    delay = (*info).current_delay_ms;
    return true;
  }
  ...
  int GetAudioLevel() {
    webrtc::internal::AudioReceiveStream* strm =
        (webrtc::internal::AudioReceiveStream*)stream_;
    return strm->GetAudioLevel();
  }

  void FlushBuffers() {
    webrtc::internal::AudioReceiveStream* strm =
        (webrtc::internal::AudioReceiveStream*)stream_;
    return strm->FlushBuffers();
  }
...
uint32_t WebRtcVoiceMediaChannel::GetPlayoutTimestamp(uint32_t ssrc)
{
  auto it = recv_streams_.find(ssrc);
  if (it == recv_streams_.end()) {
    RTC_LOG(LS_ERROR) << "Attempting to get contributing sources for SSRC:"
  ...
  return it->second->GetPlayoutTimestamp();
}

bool WebRtcVoiceMediaChannel::GetCurrentDelay(uint32_t ssrc, int& delay) {
  auto it = recv_streams_.find(ssrc);
  if (it == recv_streams_.end()) {
    RTC_LOG(LS_ERROR) << "Attempting to get contributing sources for SSRC:"
                      << ssrc << " which doesn't exist.";
  ...
  return it->second->GetCurrentDelay(delay);
}

bool WebRtcVoiceMediaChannel::GetReceiveAudioLevel(std::map<uint32_t, int>& levels) {
  levels.clear();
  for (const auto& stream : recv_streams_) {
    uint32_t ssrc = stream.first;
    int level = stream.second->GetAudioLevel();
    levels.emplace(ssrc, level);
  }
  return true;
}

void WebRtcVoiceMediaChannel::FlushBuffers(uint32_t ssrc) {
  auto it = recv_streams_.find(ssrc);
  if (it == recv_streams_.end()) {
    RTC_LOG(LS_ERROR) << "Attempting to flush buffers for SSRC:"  << ssrc
                       << " which doesn't exist.";
    return;
  }
  RTC_LOG(LS_WARNING) << "flush buffers for ssrc " << ssrc;
  return it->second->FlushBuffers();
}
