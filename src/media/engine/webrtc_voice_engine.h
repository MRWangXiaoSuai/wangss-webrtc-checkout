...
class WebRtcVoiceMediaChannel final : public VoiceMediaChannel,
 ...
 public:
  ...
  uint32_t GetPlayoutTimestamp(uint32_t ssrc) override;
  bool GetCurrentDelay(uint32_t ssrc, int& delay) override;
  bool GetReceiveAudioLevel(std::map<uint32_t, int>& levels) override;
  void FlushBuffers(uint32_t ssrc) override;
