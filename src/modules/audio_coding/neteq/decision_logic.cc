...
void DecisionLogic::SetSampleRate(int fs_hz, size_t output_size_samples) {
  ...
  RTC_LOG(LS_INFO) << "SetSampleRate sample_rate=" << sample_rate_
...
Operations DecisionLogic::GetDecision(const SyncBuffer& sync_buffer,
  ...
  const size_t cur_size_samples =
		samples_left + packet_buffer_.GetSpanSamples(decoder_frame_length, sample_rate_, true);
...
Operations DecisionLogic::FuturePacketAvailable(
  ...
  const size_t cur_size_samples =
      samples_left +
      packet_buffer_.GetSpanSamples(decoder_frame_length, sample_rate_, true);
