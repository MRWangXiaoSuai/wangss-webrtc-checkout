...
extern "C" {
#include "../../modules/audio_device/win/libavutil/channel_layout.h"
#include "../../modules/audio_device/win/libswresample/swresample.h"
}
...
bool AudioDeviceWindowsCore::CoreAudioIsSupported() {
  ...
  if (!comInit.succeeded()) {
    ...
    RTC_LOG(LS_ERROR) << "com initialize failed";
    return false;
  }
...
AudioDeviceWindowsCore::AudioDeviceWindowsCore()
    : _avrtLibrary(NULL),
      ...
      _recBlockSize(0),
      _recChannels(2),
      _audioBufferSampleRate(0),
      _audioBufferChannels(2),
      _initialized(false),
      _recording(false),
      _playing(false),
...
int32_t AudioDeviceWindowsCore::InitPlayout() {
  ...
  const int freqs[] = {48000, 32000, 16000, 8000};
  const int channels[] = {2, 1};
  bool samplerate_support = false;
  bool channel_support = false;
  ...
  } else if (pWfxOut->wFormatTag == WAVE_FORMAT_EXTENSIBLE) {
    PWAVEFORMATEXTENSIBLE pEx =
        reinterpret_cast<PWAVEFORMATEXTENSIBLE>(pWfxOut);
    pEx->SubFormat = KSDATAFORMAT_SUBTYPE_PCM;
    pEx->Samples.wValidBitsPerSample = 16;
  }

  pWfxOut->wBitsPerSample = 16;
  pWfxOut->nBlockAlign = pWfxOut->nChannels * pWfxOut->wBitsPerSample / 8;
  pWfxOut->nAvgBytesPerSec = pWfxOut->nBlockAlign * pWfxOut->nSamplesPerSec;
  ...
  if (hr == S_OK) {
    for (auto samplerate : freqs) {
      if (samplerate == static_cast<int>(Wfx.nSamplesPerSec)) {
        samplerate_support = true;
        break;
      }
    }

    for (auto channel : channels) {
      if (channel == static_cast<int>(Wfx.nChannels)) {
        channel_support = true;
        break;
      }
    }

    if (samplerate_support && channel_support) {
      ...
      _devicePlaySampleRate = Wfx.nSamplesPerSec;  // The device itself
                                                   // continues to run at 44.1
                                                   // kHz.
      _devicePlayBlockSize = Wfx.nSamplesPerSec / 100;
      _playChannels = Wfx.nChannels;
      _devicePlayChannels = Wfx.nChannels;
    } else {
      for (auto samplerate : freqs) {
        if (samplerate <= static_cast<int>(Wfx.nSamplesPerSec)) {
      ...
      _playChannels = Wfx.nChannels >= 2 ? 2 : 1;
      _playAudioFrameSize = Wfx.nBlockAlign;
      _playBlockSize = _playSampleRate / 100;
      _devicePlaySampleRate = Wfx.nSamplesPerSec;
      _devicePlayBlockSize = _devicePlaySampleRate / 100;
      _devicePlayChannels = Wfx.nChannels;
    }
  ...
  if (_ptrAudioBuffer) {
    ...
    // if (need_resample) {
    //  RTC_LOG(LS_VERBOSE) << "in samplerate : " << _playSampleRate
    //                      << ", in"
    //                         "channels : "
    //                      << _playChannels
    //                      << "out samplerate : " << Wfx.nSamplesPerSec
    //                      << "out channels : " << Wfx.nChannels;
    //  int64_t in_ch_layout = av_get_default_channel_layout(_playChannels);
    //  int64_t out_ch_layout = av_get_default_channel_layout(Wfx.nChannels);
    //  ctx = swr_alloc_set_opts(NULL,           // we're allocating a new
    //  context
    ...
    //                           _playSampleRate,     // in_sample_rate
    //                           0,                   // log_offset
    //                           NULL);
    //  if (ctx == NULL) {
    //    RTC_LOG(LS_ERROR) << "Create resample context failed";
    //    goto Exit;
    //  }
    //  if (swr_init(ctx) != 0) {
    //    RTC_LOG(LS_ERROR) << "Init resample context failed";
    //    goto Exit;
    //  }
    //  //_play_resample_context.reset(ctx);
    //  _play_resample_context = ctx;
    //}
  } else {
...
int32_t AudioDeviceWindowsCore::InitRecording() {
  ...
  Wfx.Format.nChannels = pWfxIn->nChannels;
  Wfx.Format.nSamplesPerSec = pWfxIn->nSamplesPerSec;
  ...
  //       chan < sizeof(_recChannelsPrioList) /
  ...
  if (hr == S_OK) {
    ...
    for (auto samplerate : freqs) {
      if (samplerate == static_cast<int>(Wfx.Format.nSamplesPerSec)) {
        samplerate_support = true;
        break;
      }
    }
    ...
    for (auto channel : channels) {
      if (channel == static_cast<int>(Wfx.Format.nChannels)) {
        channel_support = true;
        break;
      }
    }
    ...
    if (samplerate_support && channel_support) {
      _audioBufferSampleRate = _recSampleRate;
      _audioBufferChannels = _recChannels;
    } else {
      for (auto samplerate : freqs) {
        if (samplerate <= static_cast<int>(Wfx.Format.nSamplesPerSec)) {
          _audioBufferSampleRate = samplerate;
          break;
        }
      }
      _audioBufferChannels = Wfx.Format.nChannels >= 2 ? 2 : 1;
    }
  ...
  if (_ptrAudioBuffer) {
    // Update the audio buffer with the selected parameters
    _ptrAudioBuffer->SetRecordingSampleRate(_audioBufferSampleRate);
    _ptrAudioBuffer->SetRecordingChannels((uint8_t)_audioBufferChannels);
  } else {
    // We can enter this state during CoreAudioIsSupported() when no
    // AudioDeviceImplementation has been created, hence the AudioDeviceBuffer
    // does not exist. It is OK to end up here since we don't initiate any media
...
int32_t AudioDeviceWindowsCore::StopPlayout() {
  ...
  // if (_play_resample_context != NULL) {
  //  swr_free(&_play_resample_context);
  //  _play_resample_context = NULL;
  //}
...
DWORD AudioDeviceWindowsCore::DoRenderThread() {
  ...
  int16_t data_from_neteq[1920];  // 10ms data for 96KHz 2 channels
  SwrContext* resampler = NULL;
  if (_playChannels != _devicePlayChannels ||
      _playSampleRate != _devicePlaySampleRate) {
    RTC_LOG(LS_VERBOSE) << "in samplerate : " << _playSampleRate
                        << ", in"
                           "channels : "
                        << _playChannels
                        << ", out samplerate : " << _devicePlaySampleRate
                        << ", out channels : " << _devicePlayChannels;
    int64_t in_ch_layout = av_get_default_channel_layout(_playChannels);
    int64_t out_ch_layout = av_get_default_channel_layout(_devicePlayChannels);
    resampler = swr_alloc_set_opts(NULL,  // we're allocating a new context
                                   out_ch_layout,          // out_ch_layout
                                   AV_SAMPLE_FMT_S16,      // out_sample_fmt
                                   _devicePlaySampleRate,  // out_sample_rate
                                   in_ch_layout,           // in_ch_layout
                                   AV_SAMPLE_FMT_S16,      // in_sample_fmt
                                   _playSampleRate,        // in_sample_rate
                                   0,                      // log_offset
  ...
  //
  int playout_delay = 10 * (bufferLength / _devicePlayBlockSize) +
  ...
  while (keepPlaying) {
    ...
    while (keepPlaying) {
      ...
      if (framesAvailable < _devicePlayBlockSize) {
      ...
      const uint32_t n10msBuffers = (framesAvailable / _devicePlayBlockSize);
      for (uint32_t n = 0; n < n10msBuffers; n++) {
        // Get pointer (i.e., grab the buffer) to next space in the shared
        // render buffer.
        hr = _ptrRenderClient->GetBuffer(_devicePlayBlockSize, &pData);
        EXIT_ON_ERROR(hr);
        ...
        if (_ptrAudioBuffer) {
          ...
          if (resampler == NULL)
            ...
            nSamples =
                _ptrAudioBuffer->GetPlayoutData((int8_t*)data_from_neteq);
            uint8_t* in_data = (uint8_t*)&data_from_neteq[0];
            uint8_t* out_data = (uint8_t*)pData;
            int out_samples =
                swr_convert(resampler, &out_data, _devicePlayBlockSize,
                            (const uint8_t**)&in_data, _playBlockSize);
            if (out_samples > 0 &&
                out_samples != static_cast<int32_t>(_devicePlayBlockSize)) {
              RTC_LOG(LS_ERROR) << "resample out : " << out_samples
                                << ", need : " << _devicePlayBlockSize;
            } else if (out_samples < 0) {
              RTC_LOG(LS_ERROR) << "resample failed";
            }
          }
        }
...
DWORD AudioDeviceWindowsCore::DoCaptureThread() {
  ...
  int16_t data_to_buffer[1920];  // 10ms data for 96KHz 2 channels
  memset(data_to_buffer, 0, 3840);
  uint32_t recBufferBlockSize = _audioBufferSampleRate / 100;
  SwrContext* resampler = NULL;
  if (_recChannels != _audioBufferChannels ||
      _recSampleRate != _audioBufferSampleRate) {
    RTC_LOG(LS_VERBOSE) << "in samplerate : " << _recSampleRate
                        << ", in"
                           "channels : "
                        << _recChannels
                        << ", out samplerate : " << _audioBufferSampleRate
                        << ", out channels : " << _audioBufferChannels;
    int64_t in_ch_layout = av_get_default_channel_layout(_recChannels);
    int64_t out_ch_layout = av_get_default_channel_layout(_audioBufferChannels);
    resampler = swr_alloc_set_opts(NULL,  // we're allocating a new context
                                   out_ch_layout,           // out_ch_layout
                                   AV_SAMPLE_FMT_S16,       // out_sample_fmt
                                   _audioBufferSampleRate,  // out_sample_rate
                                   in_ch_layout,            // in_ch_layout
                                   AV_SAMPLE_FMT_S16,       // in_sample_fmt
                                   _recSampleRate,          // in_sample_rate
                                   0,                       // log_offset
                                   NULL);
    if (resampler == NULL) {
      RTC_LOG(LS_ERROR) << "Create resample context failed";
  ...
  while (keepRecording) {
    ...
    switch (waitResult) {
      ...
      case WAIT_OBJECT_0 + 1:  // _hCaptureSamplesReadyEvent
        break;
      case WAIT_TIMEOUT:  // timeout notification
        // RTC_LOG(LS_WARNING) << "capture event timed out after 0.01 seconds";
        break;
        // goto Exit;
      default:  // unexpected error
        RTC_LOG(LS_WARNING) << "unknown wait termination on capture side";
        ...
        while (syncBufIndex >= _recBlockSize) {
          if (_ptrAudioBuffer) {
            if (resampler == NULL) {
              ...
              memset(data_to_buffer, 0, 3840);
              uint8_t* in_data = (uint8_t*)syncBuffer;
              uint8_t* out_data = (uint8_t*)&data_to_buffer[0];
              int out_samples =
                  swr_convert(resampler, &out_data, 960,
                              (const uint8_t**)&in_data, _recBlockSize);
              if (out_samples > 0 &&
                  out_samples != static_cast<int>(recBufferBlockSize)) {
                RTC_LOG(LS_WARNING)
                    << "out samples : " << out_samples
                    << ", samples need : " << recBufferBlockSize;
              } else if (out_samples < 0) {
                RTC_LOG(LS_ERROR) << "audio resample failed";
              }
              _ptrAudioBuffer->SetRecordedBuffer((const int8_t*)data_to_buffer,
                                                 recBufferBlockSize);
            }
