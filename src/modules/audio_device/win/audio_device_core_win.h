  ...
  void Initialize(COINIT init) {
    ...
    if (FAILED(hr_)) {
      if (hr_ != RPC_E_CHANGED_MODE)
        RTC_LOG(LS_ERROR) << "CoInitializeEx failed, error code : " << hr_;
      else
        hr_ = S_OK;
    }
  }
...
//class SwrDeleter {
// public:
//  void operator()(SwrContext* contex) { 
//	  if (contex != NULL)
//		swr_free(&contex); 
//  }
//};
...
class AudioDeviceWindowsCore : public AudioDeviceGeneric {
 ...
 private:  // thread functions
  ...
  uint32_t _audioBufferSampleRate;
  uint32_t _audioBufferChannels;
