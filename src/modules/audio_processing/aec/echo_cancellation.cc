...
#ifdef VAD_PROCESS_RECORD
  aecpc->pre_process_in = NULL;
  aecpc->post_process_out = NULL;
#endif
  aecpc->initFlag = 0;
...
#ifdef VAD_PROCESS_RECORD
  if (aecpc->pre_process_in != NULL)
  {
    fclose(aecpc->pre_process_in);
    aecpc->pre_process_in = NULL;
  }
  if (aecpc->post_process_out != NULL)
  {
    fclose(aecpc->post_process_out);
    aecpc->post_process_out = NULL;
  }  
#endif
...
#ifdef VAD_PITCH_PROCESS
  aecpc->smooth_voice_probability = 0.0;
  aecpc->smooth_coeff = 0.0;
#endif
#ifdef VAD_PROCESS_RECORD
  if (aecpc->pre_process_in == NULL)
  {
    char filename[256];
    sprintf(filename, "pre_process_in_%p.pcm", aecpc);
    aecpc->pre_process_in = fopen(filename, "wb");
  }
  if (aecpc->post_process_out == NULL)
  {
    char filename[256];
    sprintf(filename, "post_process_out_%p.pcm", aecpc);
    aecpc->post_process_out = fopen(filename, "wb");
  }  
#endif
  // Default settings.
...
#ifdef VAD_PITCH_PROCESS
		if (aecpc->aec->nlp_mode != 3)
		{
			short near_frame[160] = { 0 };
			size_t i = 0;
			size_t j = 0;
			float voice_probability = 0.0;
			float instant_coeff = 0.0;

    for(j = 0; j < nrOfSamples; j++)
    {
      near_frame[j] = (short)(out[0][j]);
    }

#ifdef VAD_PROCESS_RECORD
  if(aecpc->pre_process_in != NULL)
  {
    fwrite(near_frame, sizeof(short), nrOfSamples, aecpc->pre_process_in); 
  } 
#endif
  aecpc->Pitchvad.ProcessChunk(near_frame, nrOfSamples, aecpc->splitSampFreq);
  voice_probability = aecpc->Pitchvad.last_voice_probability();
  aecpc->smooth_voice_probability = 0.9 * aecpc->smooth_voice_probability + 0.1 * voice_probability;
  if (aecpc->smooth_voice_probability >= 0.15 || voice_probability >= 0.15)
  {
    instant_coeff = 1.0;
  }
  else
  {
    instant_coeff = 0.125;//18dB
  }
  aecpc->smooth_coeff = 0.95 * aecpc->smooth_coeff + 0.05 * instant_coeff;

  for(i = 0; i < num_bands; i++)
  {
    for(j = 0; j < nrOfSamples; j++)
    {
      out[i][j] = out[i][j] * aecpc->smooth_coeff;
    }
  }
#ifdef VAD_PROCESS_RECORD
    for(j = 0; j < nrOfSamples; j++)
    ...
    if(aecpc->post_process_out != NULL)
    {
      fwrite(near_frame, sizeof(short), nrOfSamples, aecpc->post_process_out);  
    }
#endif
}
#endif
...
int WebRtcAec_set_config(void* handle, AecConfig config) {
  ...
  if (config.nlpMode != kAecNlpConservative &&
	  ...
	  config.nlpMode != kAecNlpMusicmode) {
    return AEC_BAD_PARAMETER_ERROR;
  }
