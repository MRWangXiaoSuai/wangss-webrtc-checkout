...
AudioProcessingImpl::AudioProcessingImpl(
  ...
  before_aec(NULL),
  before_ns(NULL),
  before_gain_control(NULL),
  before_transient(NULL), 
  before_aecm(NULL),
  far(NULL),
#endif
  ...
  if (before_aec == NULL)
  {
    before_aec = fopen("before_aec.pcm", "wb");
  }
  if (before_ns == NULL)
  {
    before_ns = fopen("before_ns.pcm", "wb");
  }
  if (before_aecm == NULL)
  {
    before_aecm = fopen("before_aecm.pcm", "wb");
  }
  if (before_gain_control == NULL)
  {
    before_gain_control = fopen("before_gain_control.pcm", "wb");
  }
  if (before_transient == NULL)
  {
    before_transient = fopen("before_transient.pcm", "wb");
  }  
  if (far == NULL)
  {
    far = fopen("apm_far.pcm", "wb");
  }  
#endif
...
AudioProcessingImpl::~AudioProcessingImpl() {
  ...
  if (before_gain_control != NULL)
  {
    fclose(before_gain_control);
    before_gain_control = NULL;
  }
  if (before_transient != NULL)
  {
    fclose(before_transient);
    before_transient = NULL;
  }
  if (before_aecm != NULL)
  {
...
void AudioProcessingImpl::ApplyConfig(const AudioProcessing::Config& config) {
  ...
  private_submodules_->echo_cancellation->set_suppression_level(
      config.echo_canceller.legacy_moderate_suppression_level
          ? EchoCancellationImpl::SuppressionLevel::kMusicSuppression
          : EchoCancellationImpl::SuppressionLevel::kHighSuppression);// for voice, aec hard coded kModerateSuppression
...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  ...
  if (before_aecm != NULL)
  {
    for (i = 0; i < capture_buffer->num_channels(); i++)
    {
      fwrite(capture_buffer->split_bands_const(0)[0], 2, capture_buffer->num_frames_per_band(), before_aecm);
    }
  }
#endif
  ...
  if (before_aec != NULL)
  {
    for (i = 0; i < capture_buffer->num_channels(); i++)
    {
      fwrite(capture_buffer->split_bands_const(0)[0], 2, capture_buffer->num_frames_per_band(), before_aec);
    }
  }
#endif
  ...
  if (before_ns != NULL)
  {
    for (i = 0; i < capture_buffer->num_channels(); i++)
    {
      fwrite(capture_buffer->split_bands_const(0)[0], 2, capture_buffer->num_frames_per_band(), before_ns);
    }
  }
#endif
  ...
  if (before_gain_control != NULL)
  {
    for (i = 0; i < capture_buffer->num_channels(); i++)
    {
      fwrite(capture_buffer->split_bands_const(0)[0], 2, capture_buffer->num_frames_per_band(), before_gain_control);
    }
  }
#endif
  // Ensure that the stream delay was set before the call to the
  ...
  if (before_transient != NULL)
  {
    for (i = 0; i < capture_buffer->num_channels(); i++)
    {
      fwrite(capture_buffer->split_bands_const(0)[0], 2, capture_buffer->num_frames_per_band(), before_transient);
    }
  }
#endif
...
int AudioProcessingImpl::ProcessRenderStreamLocked() {
  ...
  if (far != NULL)
  {
    for (i = 0; i < render_buffer->num_channels(); i++)
    {
     fwrite(render_buffer->split_bands_const(0)[0], 2, render_buffer->num_frames_per_band(), far);
    }
  }
#endif
